      ...
      <section className="grid gap-5 xl:grid-cols-[1.3fr_0.9fr]">
        <Card className="p-5">
          <div className="flex items-center justify-between gap-3">
            ...
            <Link className="text-sm text-teal-200 hover:text-teal-100" to="/projects">View all</Link>
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-2">
            {projects.map((project) => (
              <Link key={project.id} to={`/projects/${project.id}`} className="rounded-lg border border-white/10 bg-slate-950/45 p-4 transition hover:border-teal-300/30 hover:bg-teal-300/8">
      ...
      <section className="grid gap-5 lg:grid-cols-3">
        <Card className="p-5 lg:col-span-1">
          ...
          <div className="mt-4 grid gap-2">
            {recent.map((file) => (
              <Link key={file.id} to={`/files/${file.id}`} className="rounded-lg border border-white/10 bg-white/[0.035] p-3 hover:bg-white/[0.07]">
            ...
            {pinned.map((file) => (
              <div key={file.id} className="rounded-lg border border-white/10 bg-white/[0.035] p-3">
                ...
                <div className="mt-3 flex gap-2">
                  <Link className="text-xs text-teal-200 hover:text-teal-100" to={`/files/${file.id}`}>Open</Link>
                  <CopyButton label="Copy" value={file.content} />
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <Card className="p-4">
      <div className="flex items-center justify-between">
        <span className="grid size-9 place-items-center rounded-lg border border-white/10 bg-white/[0.06] text-teal-200">{icon}</span>
